...
*/
{
	...
	scanf("%s", i);
	printf(" Please output DATA file name... ");
	scanf("%s", j);
	printf(" Please binary DATA file name... ");
	scanf("%s", k);
	//i = "1.txt";
	//j = "2.txt";
...
int DLLEXPORT ENnetSize(int code)
{
	switch (code)
	{
	case 0:
...
int DLLEXPORT ENgetpipevalue(int index, int code, float *value)
{
	double a, h, q, v = 0.0;
	...
	*value = 0.0f;
	...
	if (Link[index].Type == PIPE || Link[index].Type == PUMP)
	{
		/* Retrieve called-for parameter */
		switch (code)
		{
